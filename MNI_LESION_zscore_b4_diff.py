        ...
        if os.path.exists(flair_file):
            ...
            if os.path.exists(flair_MNI):
                if os.path.split(flair_file)[-1] == os.path.split(flair_MNI)[-1]:
                    ...
                    diff = format_to_baseline_mni(flair_file,"_diff.nii.gz")
                    wm_diff = format_to_baseline_mni(flair_file,"_diff_WM.nii.gz")
                    wm_zscore = format_to_baseline_mni(flair_file,"_zscore_WM.nii.gz")

                    flair_MNI_zscore = format_to_baseline_mni(flair_MNI,"_zscore.nii.gz")
                    flair_zscore = format_to_baseline_mni(flair_file,"_zscore.nii.gz")

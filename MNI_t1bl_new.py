...
#converts the t1 MNI Baseline matrix to .mat
...
#converts the within subject aligned to T1 matrix to .mat 
...
def conv_aff(affines):
   for affine in affines:
       if not os.path.exists(affine):
           ...
           if os.path.exists(affine.split(".")[0]+ ".mat"):
               print("This affine matrix has been converted...skipping this step")
           else:
               print ("Converting the following affine from .txt to .mat format...", affine)
               cmd = ["c3d_affine_tool", "-itk", affine, "-o", affine.split(".")[0]+ ".mat"]
               proc = Popen(cmd, stdout=PIPE)
               proc.wait()
               print ("Conversion complete"); print
...
# combines the t1MNI baseline matrix with the within subject affines for the baseline scan 
    ...
    for affine in affines:
        if not os.path.exists(affine):
           print("NO AFFINES TO CONVERT - skipping this subject")
            ...
            invt.inputs.in_file2 = TP1_base_dir.replace(".txt",".mat")
...
def conv_xfm(affines,t1_mni_mat):
    for affine in affines:
        if not os.path.exists(affine):
           print("NO AFFINES TO CONVERT - skipping this subject")
        else:
            if not os.path.exists(format_to_baseline_mni(affine,"_mni.mat")):
    
                print("Combining the following affines...",affine.split(".")[0]+ ".mat", "and", t1_mni_mat.split(".")[0]+ ".mat")
                invt = fsl.ConvertXFM()
                invt.inputs.in_file = affine.split(".")[0]+ ".mat"
                invt.inputs.in_file2 =  t1_mni_mat.split(".")[0]+ ".mat"
                invt.inputs.concat_xfm = True
                invt.inputs.out_file = format_to_baseline_mni(affine,"_mni.mat")
                invt.cmdline
                invt.run()
...
#applying the combined affines from above and applying them to convert the baseline scans to T1 MNI space 
def apply_flirt(in_file, bl_t1_mni, affines):
    if os.path.exists(format_to_baseline_mni(in_file,"_T1mni.nii.gz")):
        ...
        if not os.path.exists(in_file):
            ...
            if not os.path.exists(format_to_baseline_mni(in_file,"_T1mni.nii.gz")):
                for affine in affines:
                    in_matrix = format_to_baseline_mni(affine,"_mni.mat") #format_to_baseline_mni(in_file,"_affine_mni.mat")
...
def create_mat_T1tp2_T1MNI(in_file, bl_t1_mni):
    if not os.path.exists(format_to_baseline_mni(in_file,"_T1mni.nii.gz")):
        if not os.path.exists(in_file):
            ...
            flt.inputs.reference = bl_t1_mni 
            flt.inputs.output_type = "NIFTI_GZ"
            flt.inputs.out_matrix_file = format_to_baseline_mni(in_file,"_affineMNI.mat")
            flt.inputs.out_file = format_to_baseline_mni(in_file,"_T1mni.nii.gz")
            flt.cmdline
            flt.run()
            print(flt.cmdline)
...
# this combines the within subject matrix which aligns to the T1 and the matrix with registers tp2 t1 to T1 MNI baseline
...
def apply_tp2_flirt(in_file,bl_t1_mni,t1_file, affines):
    if not os.path.exists(in_file):
        print(in_file, "this file does not exist")
    else:
        if os.path.exists(format_to_baseline_mni(in_file,"_T1mni.nii.gz")):
            ...
            for affine in affines:
                ...
                flt.inputs.apply_xfm = True
                flt.inputs.in_file = in_file
                flt.inputs.reference = format_to_baseline_mni(t1_file,"_T1mni.nii.gz")
                flt.inputs.output_type = "NIFTI_GZ"
                flt.inputs.in_matrix_file = format_to_baseline_mni(affine,"_mni.mat")
                flt.inputs.out_file = format_to_baseline_mni(in_file,"_T1mni.nii.gz")
                flt.cmdline
                flt.run()
                print(flt.cmdline)
...
def align_to_baseline(info):
    ...
    if info[1] == info[2]:
        ...
        conv_xfm(tp1.affines, t1_mni_mat)
        ...
        #apply_lesion_flirt(tp1.lst_file,tp1.bl_t1_mni)
    else:
        ...
        apply_tp2_flirt(tp2.t2_file, tp1.bl_t1_mni, tp2.t1_file, tp2.affines)
        apply_tp2_flirt(tp2.gad_file, tp1.bl_t1_mni, tp2.t1_file, tp2.affines)
        apply_tp2_flirt(tp2.flair_file, tp1.bl_t1_mni, tp2.t1_file, tp2.affines)

...
def conv_aff_mni(t1_mni_mat):
    if os.path.exists(t1_mni_mat.split(".")[0]+ ".mat"):
        print("T1 mni .mat file has been created...skipping this step")
        ...
        if not os.path.exists(affine):
           print("NO AFFINES TO CONVERT - skipping this subject")
            ...
            if not os.path.exists(format_to_baseline_mni(affine,"_mni.mat")):
                ...
                print("Combining the following affines...",affine.split(".")[0]+ ".mat", "and",TP1_base_dir + "/mni_angulated/affine.mat")
                invt = fsl.ConvertXFM()
...
def create_mat_T1tp2_T1MNI(in_file, bl_t1_mni):
    if not os.path.exists(format_to_baseline_mni(in_file,"_T1mni.nii.gz")):
        if not os.path.exists(in_file):
            ...
            flt.inputs.dof = 6 
            flt.inputs.in_file = in_file # in file is tp2.t1_file
            flt.inputs.reference = bl_t1_mni 
            flt.inputs.output_type = "NIFTI_GZ"
    ...
    for affine in affines:
        if not os.path.exists(affine):
           print("NO AFFINES TO CONVERT - skipping this subject")
            ...
            invt.inputs.concat_xfm = True
            invt.inputs.out_file = format_to_baseline_mni(affine,"_affine_mni.mat")
            invt.cmdline
            invt.run()
            print(invt.cmdline)
            print ("Transformation complete"); print()
    ...
    for affine in affines: 
        if not os.path.exists(in_file):
            ...
            if os.path.exists(format_to_baseline_mni(in_file,"_T1mni.nii.gz")):
                ...
                in_matrix = format_to_baseline_mni(affine,"_affine_mni.mat")
                print ("Applying FLIRT to the following file...")
                print (in_file)
                flt = fsl.FLIRT()
                flt.inputs.cost = "mutualinfo"
...
def align_to_baseline(info):
    ...
    if info[1] == info[2]:
        ...
        apply_flirt(tp1.lst_file, tp1.bl_t1_mni)

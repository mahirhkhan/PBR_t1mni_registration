...
def apply_lesion_flirt(lst_file, flair_file, t1_file):
    ...
    if lst.startswith("no_FP"):
        ...
        flt.inputs.reference = format_to_baseline_mni(flair_file, "_T1mni.nii.gz") #bl_t1_mni format_to_baseline_mni(t1_file,"_T1mni.nii.gz") format_to_baseline_mni(flair_file,"_T1mni.nii.gz")
        flt.inputs.output_type = "NIFTI_GZ"
        #affine_mni = format_to_baseline_mni(affine,"_mni_FINAL.mat")
        ...
        #ffine_mni = format_to_baseline_mni(t1_file,"_T1mni.nii.gz").replace("_T1mni.nii.gz","_affineMNI.mat")
        flt.inputs.in_matrix_file = format_to_baseline_mni(t1_file,"_affineMNI.mat")
...
def register_wm_mask(in_file, bl_t1_mni):
    ...
    for msid_mse in os.listdir(gtech_path):
        if msid_mse.startswith(msid + "-" + mse):
            ...
            flt.inputs.output_type = "NIFTI_GZ"
            flt.inputs.apply_xfm = True
            wm_MNI = os.path.split(format_to_baseline_mni(in_file,"_affineMNI.mat"))[0] + "/WM_MNI.nii.gz"
            ...
            flt.inputs.output_type = "NIFTI_GZ"
            flt.inputs.apply_xfm = True
            gm_MNI = os.path.split(format_to_baseline_mni(in_file,"_affineMNI.mat"))[0] + "/GM_MNI.nii.gz"
...
"""def register_wm_mask2(in_file): #, flair_file):
    new_in = os.path.split(in_file)[-1]
    msid = new_in.split("-")[0]
    mse = new_in.split("-")[1]
    ...
    for msid_mse in os.listdir(gtech_path):
        if msid_mse.startswith(msid + "-" + mse):
            ...
            flt.inputs.in_file = gtech_path + "/"+ msid_mse + "/wm_mask.nii.gz" 
            flt.inputs.reference = format_to_baseline_mni(in_file,"_T1mni.nii.gz")
            flt.inputs.in_matrix_file = format_to_baseline_mni(in_file,"_affineMNI.mat")#format_to_baseline_mni(flair_file, "_affine_mni_FINAL.mat")
            flt.inputs.output_type = "NIFTI_GZ"
            flt.inputs.apply_xfm = True
            #flt.inputs.out_matrix_file = combined_affine
...
def align_to_baseline(info):
    ...
    if info[1] == info[2]:
        ...
        apply_lesion_flirt(tp2.lst_file,tp2.flair_file, tp2.t1_file)

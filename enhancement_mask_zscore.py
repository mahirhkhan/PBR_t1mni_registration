...
def sub_gad_nogad(gad_file, t1_file, mseid):
    ...
    if os.path.exists(gad_mni):
        shutil.copyfile(gad_mni,"/data/henry7/PBR/subjects/"+ mseid+ "/enhancement_mask/"+os.path.split(gad_mni)[1])
...
def gad_mask(mseid, msid):
    ...
    if os.path.exists(gad_dir):
        ...
        if os.path.exists(wm_mask):
            ...
            thresh.inputs.out_file =  PBR_base_dir + mseid + '/enhancement_mask/gad_mask_thr1.nii.gz'
            thresh.cmdline
            thresh.run()

            cmd = ["fslview", PBR_base_dir+ mseid+ "/enhancement_mask/*.nii.gz"]
            proc = Popen(cmd, stdout=PIPE)
            output = [l.decode("utf-8").split() for l in proc.stdout.readlines()[:]]

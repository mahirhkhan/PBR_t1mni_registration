...
def create_flair_lesions(mseid, msid):
    ...
    if not os.path.exists(mni_long):
        ...
        if os.path.exists(flair_file):
            ...
            cmd = ["fslmaths", base_dir + "/skull.nii.gz", "-uthrP","40","-bin",base_dir+ "/thr.nii.gz"]
            proc = Popen(cmd, stdout=PIPE)
            output = [l.decode("utf-8").split() for l in proc.stdout.readlines()[:]]
            ...
            cmd = ["fslmaths", lesion_bin_MNI,"-sub", base_dir + "/thr2.nii.gz", "-thr", ".1", "-bin", "-mul",flair_file,  lesion_mul_flair]
            proc = Popen(cmd, stdout=PIPE)
            output = [l.decode("utf-8").split() for l in proc.stdout.readlines()[:]]
            ...
            # lesion histogram 
                ...
                cmd = ["fslmaths",base_dir+ "/lesion_prob_map.nii.gz", "-dilM", "-mul", prob_map, "-thr", ".9", "-bin", base_dir+ "lesion_prob_map.nii.gz"]
                proc = Popen(cmd, stdout=PIPE)
                output = [l.decode("utf-8").split() for l in proc.stdout.readlines()[:]]
